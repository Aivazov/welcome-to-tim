import React from 'react';
import { motion } from 'framer-motion';

type Props = {};

export default function BackgroundCircles({}: Props) {
  return (
    <motion.div
      initial={{
        opacity: 0,
      }}
      animate={{
        scale: [0.8, 1.1, 1.3, 1.5, 1],
        opacity: [0.1, 0.2, 0.4, 0.8, 0.1, 1],
        borderRadius: ['20%', '20%', '50%', '80%', '20%'],
      }}
      transition={{
        duration: 2,
      }}
      className="flex justify-center items-center relative z-0"
    >
      <div className="absolute border border-[#ccc] rounded-full h-[200px] w-[200px] mt-52 animate-ping" />
      <div className="absolute border border-[#333] rounded-full h-[300px] w-[300px] mt-52" />
      <div className="absolute border border-[#333] rounded-full h-[500px] w-[500px] mt-52" />
      <div className="absolute border border-[#664711] rounded-full h-[650px] w-[650px] mt-52 animate-pulse z-0" />
      <div className="absolute border border-[#333] rounded-full h-[800px] w-[800px] mt-52" />
    </motion.div>
  );
}
